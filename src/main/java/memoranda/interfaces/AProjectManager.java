...
//            _root.appendChild(new Comment("This is JNotes 2 data file. Do not modify."));
    ...
    public static AProject getProject(String id) {
    ...
    public static Vector getActiveProjects() {
        ...
        Vector v = new Vector();
        for (int i = 0; i < prjs.size(); i++) {
            AProject prj = new IProjectImpl((Element) prjs.get(i));
            if (prj.getStatus() == AProject.ACTIVE)
                v.add(prj);
        }
        return v;
    }
    ...
    public static AProject createProject(String id, String title, CalendarDate startDate, CalendarDate endDate) {
        Element el = new Element("project");
        el.addAttribute(new Attribute("id", id));
        _root.appendChild(el);
        AProject prj = new IProjectImpl(el);
        prj.setTitle(title);
    ...
    public static AProject createProject(String title, CalendarDate startDate, CalendarDate endDate) {
        return createProject(Util.generateId(), title, startDate, endDate);
    }
    
    public static void removeProject(String id) {
        AProject prj = getProject(id);
        if (prj == null)
            return;
        AHistory.removeProjectHistory(prj);
