...
public class IResourcesListImpl implements IResourcesList {
	...
	private AProject _project = null;
     ...
     */
    public IResourcesListImpl(Document doc, AProject prj) {
        _doc = doc;
        _root = _doc.getRootElement();
        _project = prj;
    }

    public IResourcesListImpl(AProject prj) {
    ...
    public Vector getAllResources() {
        ...
        for (int i = 0; i < rs.size(); i++)
            v.add(new AResource(rs.get(i).getAttribute("path").getValue(), rs.get(i).getAttribute("isInetShortcut") != null, rs.get(i).getAttribute("isProjectFile") != null));
        return v;
    }
     ...
     */
    public AResource getResource(String path) {
        Elements rs = _root.getChildElements("resource");
        for (int i = 0; i < rs.size(); i++)
            if (rs.get(i).getAttribute("path").getValue().equals(path))
                return new AResource(rs.get(i).getAttribute("path").getValue(), rs.get(i).getAttribute("isInetShortcut") != null, rs.get(i).getAttribute("isProjectFile") != null);
        return null;
    }
