...
public class AHistory {
    ...
    public static void add(AHistoryItem item) {
        if (prev != null)   
    ...
    public static AHistoryItem rollBack() {        
        Object n = prev;        
        if (p > 1) {                          
            p--;
            prev = _list.get(p-1);
        ...
        if (p < _list.size() - 1)
            ...
            next = null;         
        return (AHistoryItem)n;
    }

    public static AHistoryItem rollForward() {
        Object n = next;        
        if (p < _list.size() - 1) {
            p++;
            if (p == 1) 
    ...
    public static void removeProjectHistory(AProject prj) {
        Vector list = new Vector();
        String id;
        
        for (int i = 0; i < _list.size(); i++) {
            id = (((AHistoryItem) _list.elementAt(i)).getProject()).getID();
    ...
    private static void notifyListeners(AHistoryItem n) {
    ...
    static class HistoryBackAction extends AbstractAction {
		...
		Date date = ((AHistoryItem) prev).getDate().getDate();
		    putValue(
...
//                putValue(Action.SHORT_DESCRIPTION, Local.getString("Back to") + " " + ((HistoryItem) prev).getDate().toString());
    ...
    static class HistoryForwardAction extends AbstractAction {
		...
		SimpleDateFormat sdf = new SimpleDateFormat();
		sdf = (SimpleDateFormat)DateFormat.getDateInstance(DateFormat.SHORT);
		Date date = ((AHistoryItem) next).getDate().getDate();
