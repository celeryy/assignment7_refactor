...
public class ACurrentProject {

    private static AProject _project = null;
    ...
    static {
        ...
        //ProjectManager.init();
        _project = AProjectManager.getProject(prjId);
		
		if (_project == null) {
			// alexeya: Fixed bug with NullPointer when LAST_OPENED_PROJECT_ID
			// references to missing project
			_project = AProjectManager.getProject("__default");
			if (_project == null) 
				_project = (AProject)AProjectManager.getActiveProjects().get(0);						
    ...
    public static void set(AProject project) {
    ...
    private static void notifyListenersBefore(AProject project, INoteList nl, ITaskList tl, IResourcesList rl) {
