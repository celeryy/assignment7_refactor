...
//    public static final String NS_JNNL = "http://www.openmechanics.org/2003/jnotes-noteslist";
     ...
     */
    public INoteListImpl(Document doc, AProject prj) {
        _doc = doc;
        _root = _doc.getRootElement();
        _project = prj;
    }

    public INoteListImpl(AProject prj) {
            ...
            for (int mi = 0; mi < ms.size(); mi++) {
                ...
                for (int di = 0; di < ds.size(); di++) {
					...
					for(int ni = 0; ni < ns.size(); ni++) {
						...
						INote n = new INoteImpl(ne.getElement(), _project);
    ...
    public INote getNoteForDate(CalendarDate date) {
		...
		if(ns.size()>0) {
			...
			INote currentNote = new INoteImpl(n.getElement(), _project);
			return currentNote; 
		}
		return null;
