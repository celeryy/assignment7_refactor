...
public class TaskTableSorter extends TaskTableModel{
	...
	public Object getChild(Object parent, int index) {
		...
		if (parent instanceof AProject){
			if( activeOnly() ) c = ACurrentProject.getTaskList().getActiveSubTasks(null, CurrentDate.get());
			else c = ACurrentProject.getTaskList().getTopLevelTasks();
		}
		else{
			ITask t = (ITask) parent;
			if(activeOnly()) c = ACurrentProject.getTaskList().getActiveSubTasks(t.getID(), CurrentDate.get());
